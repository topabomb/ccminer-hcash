...
extern void x14_sm3_cpu_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_hash, int order);




// X14 CPU Hash
extern "C" void x14hsrhash(void *output, const void *input)
{
...
extern "C" int scanhash_hsr(int thr_id, struct work* work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	if (!init[thr_id])
	{
		...
		x14_sm3_cpu_init(thr_id, throughput);
...
// cleanup
extern "C" void free_x14_hsr(int thr_id)
{
	if (!init[thr_id])
		return;
